...
// Make sure you define this *before* loading hardgame.js in your HTML:
// <script>const QUIZ_ID = 'QZ03';</script>
  ...
  function step() {
    if (idx < path.length) {
      ...
      if (snakePaths[p1sum]) {
        ...
        setTimeout(() => {
          slideToTarget(p1sum, jumps[p1sum]);
        }, 400);
      } else if (p1sum === 100) {
        handleWin();
      }
...
function submitChoice() {
  ...
  fetch("../student/check-answer.php", {
    ...
    .then(data => {
      ...
      console.log("Saving progress from submitChoice, position:", p1sum, "score:", score);
      saveProgressToDatabase();
    ...
    .catch(err => {
      ...
      // Still try to move if correct answer despite error
      if (confirm("Communication error. Continue anyway?")) {
        play(num);
      }
    });
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (isNewGame || wasCompleted) {
    ...
    // For a brand new game, forcefully create a new record
    console.log("Creating new game record for hard game");
    saveProgressToDatabase(true);
    
    // For tracking further progress
